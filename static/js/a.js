$(document).ready(function(){
  ...
  $(window).resize(function() {
    ...
    return resizing = setTimeout(doneResizing, 200);
  });
  ...
  // Trying to make some things appear
  //$('div#dataSource').text("working!")
  //$('dataSource').innerHTML = "Your text";
  //for each(var fieldName in ['sourceName','sourceURL','itemName']    
  ...
  var addRow = function(copy, row){
    ...
    if(row)
    {
      var searchRow = _.find($("tr.search-row"), function(v){return $(v).data('row') == row;})
    ...
    {
      //      console.log('newRow');
      var newRow = _.find($("tr.search-row"), function(v){return $(v).data('row') == rows;})
    ...
    if(!row)
    {
      $('#category_box_template').clone()
        .removeClass('category_box_template')
        .addClass('category_box')
        .attr('id', "cat_box_" + rows)
        .appendTo('#search_queries table.top tr:last td.box-td');
    }
    else {
    ...
    if(row) 
      last_cat = _.find($('.edit-box'), function(v){ return $(v).data('row') == row;});
    ...
    $('#search_queries').on('click', '#cat_box_' + rows + ' a.box_data', function(event){
      ...
      $(_.filter($('.edit-box'),function(el){return $(el).data('row')!=row;}))
        .each(function(i,v) {
          $(v).hide();
        });
  ...
  var newEditBox = function(num) {
    ...
    var divHTML = $('<div></div>')
      ...
      .data('row', num)
      .css('display','none')
      .html('<span class=subtitle>Restrict search to the following ' + options['settings']['itemName'] + 's</span>');
    
    // add options table
    var table = "<table></table>";
    var datatypes = ['categorical'];//, 'hierarchical'];
    var opts = _.filter(options['ui_components'], function(v){return _.include(datatypes, v['type']);});
    _.each(opts, function(opt) {
      ...
      if (opt['type'] == 'categorical')
      {
        ...
        selectHTML += "</select>";
        select = _(selectHTML).template({elts: elts});
      }
      ...
      var row = _(rowHTML).template({label: opt['name'], select: select, dbcode: opt['dbfield']});
  ...
  var fixEditBoxPositions = function() {
    _($('.edit-box')).each(function(v,i){
      ...
      metaBox = $(_.filter($('tr.search-row'), function(r){return $(r).data('row')==i;}))
        .find('.category_box');
  ...
  var addButtonHTML =  '<img height="12px" src="static/images/plus.png" class=add-query>';
  ...
  getSmoothing = function(intval) {
    ...
    else if (intval == 11) { return 12;}
    else if (intval == 12) { return 18;}
    else if (intval == 13) { return 24;}
    else {return 36;}
  };
  ...
  buildQuery = function() {
    ...
    $('.search-row').each(function(i, v){
      var row = $(v).data('row');
      var edit_box = _.find($('.edit-box'), function(el){return $(el).data('row') == row;});
      var subcats = {};
  ...
  renderChart = function() {
      ...
      _.each(years, function(year){
        ...
        if (myt == "date_month" || myt == "month") {
          ...
          var opts = {
            n: i,
            t: year,
            str: date.toDateString().substring(3)
          };
          sdata.push({x: date.getTime(), y:parseFloat(vals[year]), opts: opts});
        }
          ...
          sdata.push({x: Date.UTC(year, 0, 1), y:parseFloat(vals[year]), opts: opts});
        }
        else {
          var opts = {
            n: i,
    ...
    var chart = new Highcharts.Chart({
      ...
      xAxis: {
        ...
        //        tickPositions: _([1995, 2000, 2005, 2010, 2012]).map(function(v) {return Date.UTC(v,1,1);}),
        //        min: minTime(),
        //        max: maxTime(),
    lineWidth: 1,
        lineColor: '#555555',
      ...
      legend: {
        ...
        labelFormatter: function() {
          return '<span style="color: '+ this.color +'; font-weight: bold">'+this.name + ' ' +q[this.index]+'</span>' ;
        },
        itemStyle: {
          fontSize: '120%'
        }
      ...
      tooltip : {
        formatter: function() {
          ...
          return "<b>" + point.series.name + "</b><br />" + point.opts.str + " <br />Freq: " + point.y + "<br /><b>Click for " + options['settings']['itemName'] + "s</b>";
        }
      }
    });
  }
  ...
  showBooks = function(event) {
    ...
    var name = this.name;
    var title= "Top search results for " + name + " in " + event.point.opts.str;
    ...
    //    console.log(JSON.stringify(query));
    //    console.log("Posting query to bindings");
    return $.ajax({
      url: '/cgi-bin/dbbindings.py',
      type: 'post',
      data: {
  ...
  validateQuery = function() {
    ...
    if(lengthArray.length > 0) 
      error = "Sorry, Bookworm currently only supports one and two-word phrases.";
